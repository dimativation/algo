...
def main (ticker, timeframe, strategy):
    ...
    figure = cerebro.plot()[0][0].savefig('data/TSLA/example.png').close()
    testresults_dict['total_return'] = 100 * (cerebro.broker.getvalue() - 100000)/100000
    testresults_dict['final_value'] = cerebro.broker.getvalue()
        ...
        for ticker in tickers:
            for timeframe in timeframes:
                try:
                    ...
                    one_result_dict = main (ticker.upper(), timeframe, strategy)
                    log_df = log_df.append(one_result_dict, ignore_index=True)
                except:
                    print("not working", ticker, timeframe)
    is_better(log_df)                
    print(log_df.tail())
